...
export const getStats = async (_req: Request, res: Response) => {
  try {
    ...
    const artistasPorGenero = await Artist.aggregate([
      ...
      { $project: { genero: "$_id", _count: { genero: "$count" }, _id: 0 } },
    ]);

    const allArtists = await Artist.find({}, "birthDate").lean();
    const faixasEtarias = { "18-25": 0, "26-35": 0, "36-45": 0, "46-60": 0, "60+": 0 };
    ...
    const topCategorias = await Artist.aggregate([
      ...
      { $sort: { count: -1 } },
      { $limit: 5 },
      { $project: { categoria: "$_id", total: "$count", _id: 0 } },
    ]);

    res.json({
      totais: { totalArtistas, totalColetivos, totalEquipamentos },
      artistasPorGenero,
      artistasFaixaEtaria: faixasEtarias,
      artistasDiversidade: { lgbtqiapn, negros, indigenas, pcd },
      topCategorias,
    });
  } catch (error) {
